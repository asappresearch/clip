...
def sent_tokenize(text):
    ...
    segments = text.split("\n\n")
    idxs = [text.find(seg) for seg in segments]
    ...
    for i in range(len(segments)):
        ...
        if (
            ...
            if (i == N - 1) or is_title(segments[i + 1]):
                ...
                # use exact indexes so we can map back to document positions
                new_segments.append(text[text.index(segments[i-1]) : text.index(segments[i])+len(segments[i])])
            else:
                new_segments.append(segments[i])
